...
TEST_CASE("Movement parser - Movimentos básicos", "[movement][parser]") {
  SECTION("Front move") {
    Movement move("F");
    REQUIRE(move.move == FRONT);
    REQUIRE(move.type == CLOCK_WISE);
  }

  SECTION("Back move") {
    Movement move("B");
    REQUIRE(move.move == BACK);
    REQUIRE(move.type == CLOCK_WISE);
  }

  SECTION("Up move") {
    Movement move("U");
    REQUIRE(move.move == UP);
    REQUIRE(move.type == CLOCK_WISE);
  }

  SECTION("Down move") {
    Movement move("D");
    REQUIRE(move.move == DOWN);
    REQUIRE(move.type == CLOCK_WISE);
  }

  SECTION("Left move") {
    Movement move("L");
    REQUIRE(move.move == LEFT);
    REQUIRE(move.type == CLOCK_WISE);
  }

  SECTION("Right move") {
    Movement move("R");
    REQUIRE(move.move == RIGHT);
    REQUIRE(move.type == CLOCK_WISE);
  }
}
